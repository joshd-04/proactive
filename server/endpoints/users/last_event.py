...
def get_user_last_event(db_pool, username):
    # This SQL is extended from the one in events.py
    LAST_EVENT_SQL = """SELECT events.* FROM user_event_participation INNER JOIN events ON user_event_participation.event_id = events.event_id WHERE user_event_participation.username = %s AND events.date_time < NOW() ORDER BY events.date_time DESC LIMIT 1;"""
    ...
    result_dict = {}
    try:
        cursor.execute(LAST_EVENT_SQL, (username,))
        ...
        if result:
            ...
            cursor.execute(GET_NUMER_OF_PARTICIPANTS_SQL, (result_dict["event_id"],))
            ...
            result_dict["participant_count"] = participant_count[0]
